...
class DistriLock : boost::noncopyable {
  public:
  DistriLock() : mutex_(), zkConnStr_(""), isInit_(false),ready_(false), childNodeName_("") {}
  ...
  bool lock() {
    ...
    if (childNodes.front() == retpath_) {
      ...
      if (zkClient_->regNodeWatcher(path,
                                    std::bind(&DistriLock::regWatcher_cb, this,
                                              _1, _2, _3, _4, _5, _6),
                                    nullptr) == false) {
...
void distriLocktest_1(DistriLock& d1) {
  d1.lock();
...
void distriLocktest_2(DistriLock& d2) {
  d2.lock();
